...
exports.handler = async function (event) {
  ...
  console.log("submitLambda", event);

  const { input, token } = event;

  // mock a long-running task executed successfully
  ...
  // mock a long-running task failure
  const params = {
    output: JSON.stringify({
      status: "FAILED",
      error: {
  ...
  // below code will fail the entire workflow
